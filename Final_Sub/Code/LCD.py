...
#Creating a thread lock so that only one thread can display at a time. 
...
class myThread (threading.Thread):
	...
	def __init__(self,threadID,name,q):
		...
		self.name = name
        # A printing function is passed to the thread that it should carry out when running
		self.q = q
    #Run method for the thread
	def run(self):
        #Current active thread acquires the threadlock, so only one thread prints at a time
		threadlock.acquire()
